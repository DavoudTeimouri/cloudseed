...
Available modules (toggle by number + `Enter` to confirm, `c` to configure selected, `a`=all, `n`=none, `0`=back):
...
Commands: [number] toggle + Enter=confirm | c=configure | a=all | n=none | 0/Esc=back
```
